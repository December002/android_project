...
/*
    어떤 데이터를 index 로 관리를하고 readOnly 로 사용한다면
    List 를 사용하면 된다
 */

fun main(){
    val names:List<String> = listOf<String>("kim", "lee", "park")
    // value 의 generic 이 추론 가능하기 때문에 생략가능
    // name2 의 type 이 추론 가능하기 때문에 생략 가능
    val name2 = listOf<String>("kim", "lee", "park")

    val nums:List<Int> = listOf(10, 20, 30)

    val info:List<Any> = listOf(10, "kim", true)

    // names List 의 0번 index 참조하는 방법1
    val a=names.get(0)
    ...
    println("-------")
    //위의 forEach() 함수를 줄인 표현식으로 사용하면 아래와 같다
    names.forEach{
        println(it)
    }
    ...
    println("List 의 item을 역순으로 참조하고 싶다면")
